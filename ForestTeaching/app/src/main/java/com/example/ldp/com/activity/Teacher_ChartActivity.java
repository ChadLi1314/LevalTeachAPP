...
public class Teacher_ChartActivity extends BaseActivity {
    ...
    public static void actionstart(Context context,String t_id){
        Intent intent = new Intent(context,Teacher_ChartActivity.class);
    ...
    private void initFruits(){
        ...
        map.put("t_id",t_id);
        String str = HttpGetUtil.SendPostHttp("teacher/toShowCourtyardPieChartjsp.action",map);
        Gson gson = new Gson();
        analyzesList = gson.fromJson(str,new TypeToken<List<Analyzes>>(){}.getType());
        if(analyzesList.size()==0){
            Toast.makeText(Teacher_ChartActivity.this,"服务器没数据",Toast.LENGTH_SHORT).show();
        }
    }
}
